...
type HumanReadableRecord struct {
	...
	SolutionWord         string               `json:"solution_word"`
...
func readHumanReadableRecord(reader io.Reader, index int, header BinaryHeader) (HumanReadableRecord, error) {
	...
	for i := 0; i < int(record.TurnDepth); i++ {
		...
		previousTurns = append(previousTurns, HumanReadableTurn{
			Turn:           i + 1,
			GuessWord:      wordBytesString(record.PreviousGuessWords[i]),
			Feedback:       feedback,
	...
	for i := 0; i < int(header.TopK); i++ {
		labels = append(labels, HumanReadableLabel{
			Rank:           i + 1,
			GuessWord:      wordBytesString(record.TopKGuessWords[i]),
			ReductionRatio: record.TopKReductionRatios[i],
			WorstCaseSize:  record.TopKWorstCaseSizes[i],
		})
	}

	solutionWord := wordBytesString(record.SolutionWord)
	return HumanReadableRecord{
		Index:                index,
		SolutionWord:         solutionWord,
		OpeningState:         solutionWord == "",
...
func readBinaryRecord(reader io.Reader) (Record, error) {
	var record Record
	if _, err := io.ReadFull(reader, record.SolutionWord[:]); err != nil {
		return Record{}, err
	}
	if err := binary.Read(reader, binary.LittleEndian, &record.TurnDepth); err != nil {
		return Record{}, err
	}
	for i := range record.PreviousGuessWords {
		if _, err := io.ReadFull(reader, record.PreviousGuessWords[i][:]); err != nil {
			return Record{}, err
		}
	}
	...
	for i := range record.TopKGuessWords {
		if _, err := io.ReadFull(reader, record.TopKGuessWords[i][:]); err != nil {
			return Record{}, err
		}
	}
